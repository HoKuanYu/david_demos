...
static void app_deinit(AppObj *obj)
{
    tivxDavidTiovxTestUnLoadKernels(obj->context);
...
static vx_status app_create_graph(AppObj *obj)
{
    ...
    VX_PRINT(VX_ZONE_INIT, "App Create Graph\n");
    ...
    if (status == VX_SUCCESS)
    {
        obj->david_gray = vxCreateVirtualImage(obj->graph, obj->width, obj->height, (vx_df_image)VX_DF_IMAGE_U8);
        status = vxSetReferenceName((vx_reference)obj->david_gray, "VirtualDavidGrayU8");
    }
    ...
    if (status == VX_SUCCESS)
    {
        ...
        VX_PRINT(VX_ZONE_INIT, "App Create Channel Extract Node Done\n");
    }

    if (status == VX_SUCCESS)
    {
        obj->david_node = tivxDavidTiovxTestNode(obj->graph, obj->gray, obj->david_gray);
        status = vxSetReferenceName((vx_reference)obj->david_node, "DavidNode");
        if (status == VX_SUCCESS)
        {
            status = vxSetNodeTarget(obj->david_node, VX_TARGET_STRING, TIVX_TARGET_MCU2_0);
        }
        VX_PRINT(VX_ZONE_INIT, "App Create David TIOVX Node Done\n");
    }

    if (status == VX_SUCCESS)
    {
        obj->sobel_node = vxSobel3x3Node(obj->graph, obj->david_gray, obj->grad_x, obj->grad_y);
        ...
        VX_PRINT(VX_ZONE_INIT, "App Create Sobel Node Done\n");
    }
    ...
    if (status == VX_SUCCESS)
    {
        ...
        VX_PRINT(VX_ZONE_INIT, "App Create Magnitude Node Done\n");
    }

    VX_PRINT(VX_ZONE_INIT, "App Create Graph Done\n");
...
static vx_status app_init(AppObj *obj)
{
    ...
    if (status == VX_SUCCESS)
    {
        tivxDavidTiovxTestLoadKernels(obj->context);
    }
